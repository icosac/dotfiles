...
LIST=(neovim vim tmux zsh git) #List of all possible packets to install
...
if in_list "${packages[*]}" "neovim"; then
  ...
  $IN build-essential make automake cmake pkg-config libtool libtool-bin python3 python3-pip gettext unzip latexmk &> /dev/null
  python3 -m pip install -U neovim
  if [ -d neovim ]; then rm -rf neovim; fi
  $CLONE -b stable https://github.com/neovim/neovim
  cd neovim 
  make CMAKE_BUILD_TYPE=Release -j &> /dev/null
